...
"""
The problem is 
-d2u/dx^2 + u(x) = exp(-x) on the [0, inf] interval with dirichlet BC set to u(0) = 0, u(inf) = 0
...
def fun(approximationOrder, amountOfElements, integrationPointsAmount = 500):
    ...
    analyticSolution = lambda x: 0.5 * np.exp(-x)*x
    analyticSolutionD2 = lambda x: -np.exp(-x) + np.exp(-x) * x * 0.5
    ...
    # plt.plot(grid, -d2gridSol + gridSol, label="lhs")
    plt.plot(grid, d2gridSol, label="second derivative")
    plt.plot(grid, analyticSolutionD2(grid), label="analytic second derivative")
    # plt.plot(grid, np.exp(-grid), label="rhs")
    plt.legend()
    plt.show()
