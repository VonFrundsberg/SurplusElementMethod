...
"""
The problem is 
-d2(u/dx2) - d(u/dx) + u(x) = exp(-x) on the [0, inf] interval with dirichlet BC set to u(0) = 0.0, u(inf) = 0.0
"""
def fun(approximationOrder, amountOfElements, integrationPointsAmount = 500):
    ...
    numericalRHS = -d2gridSol - d1gridSol + gridSol
    rhsX = np.exp(-grid)
    print(approximationOrder, np.max(np.abs(gridSolution - analyticSolution(grid))), np.max(np.abs(numericalRHS - rhsX))/np.max(np.abs(rhsX)))
