...
"""
The problem is 
-d2(u/dx2) - d(u/dx) + u(x) = exp(-x) on the [0, 10] interval with dirichlet BC set to u(0) = 0.0, u(10) = 0.0
"""
def fun(approximationOrder, amountOfElements, integrationPointsAmount = 500):
    ...
    galerkinMethodObject.setBilinearForm(innerForms=[gradForm, fluxForm, innerForm],
    ...
    """Zero Dirichlet BC"""
    # analyticSolution = lambda x: np.exp(-x) - np.exp((-0.5 - np.sqrt(5.0)/2.0) * x)
    sq5 = np.sqrt(5.0)
    analyticSolution = lambda x: \
        np.exp(10.0 * sq5 - x) / (-1.0 + np.exp(10.0 *sq5)) - np.exp(-x)/(-1.0 + np.exp(10.0 * sq5)) + \
        np.exp((-0.5 + sq5/2.0) * x) / (-1.0 + np.exp(10.0 * sq5)) + np.exp(-5.0 + 5.0 * sq5 + (-0.5 - sq5/2.0) * x) / (-1.0 + np.exp(10.0 * sq5)) - \
        np.exp(10.0 * sq5 + (-0.5 - sq5/2.0) * x) / (-1.0 + np.exp(10.0 * sq5)) - np.exp(-5.0 + 5.0 * sq5 + (-0.5 + sq5/2.0) * x) / (-1.0 + np.exp(10.0 * sq5))
