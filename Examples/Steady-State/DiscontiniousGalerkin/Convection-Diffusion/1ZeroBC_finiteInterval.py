...
"""
The problem is 
-d2(u/dx2) - d(u/dx) = exp(-x) on the [0, 10] interval with dirichlet BC set to u(0) = 0.0, u(10) = 0.0
"""
def fun(approximationOrder, amountOfElements, integrationPointsAmount = 500):
    ...
    eps = 1e-14
    sigma = 0 * approximationOrder ** 2 / (domainSize / amountOfElements)
    ...
    """Zero Dirichlet BC"""
    analyticSolution = lambda x: (-10.0/(-1.0 + np.exp(10.0)) + 10.0 * np.exp(-x)/(-1.0 + np.exp(10.0)) +
                                  np.exp(10.0 - x) * x/(-1.0 + np.exp(10.0)) - np.exp(-x) * x/(-1.0 + np.exp(10.0)))
    # """Zero Neumann BC"""
    # analyticSolution = lambda x: (9 - 9 * np.exp(-x) + np.exp(-x) * x)
