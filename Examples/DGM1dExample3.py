...
The problem is 
d2u/dx^2 = -pi**2 cos(pi * x) on the [-1, 1] interval with dirichlet BC set to u(-1) = -1.0, u(1) = -1.0
with weak dirichlet boundary conditions
...
def fun(approximationOrder, amountOfElements, integrationPointsAmount = 500):
    ...
    functional = lambda testElement: elem1dUtils.integrateFunctional(
        testElement=testElement, function=lambda x: np.pi**2 * np.cos(np.pi * x),
    ...
    boundaryFunctional0 = lambda testElement: elem1dUtils.evaluateFunctionalAtBoundaries0(
        testElement=testElement, weight=lambda x: x,
        leftValue=1.0, rightValue=1.0, leftBoundary=-1.0, rightBoundary=1.0)
    boundaryFunctional1 = lambda testElement: elem1dUtils.evaluateFunctionalAtBoundaries1(
        testElement=testElement, weight=lambda x: x * 0.0 - sigma,
        leftBoundary=-1.0, rightBoundary=1.0)
...
fun(55, 1, integrationPointsAmount=10000)