...
defectsApproximationOrder: int = 50
integrationPointsAmount: int = 2000
domainSize = 200
...
def defectForms(v, g, l):
    ...
    def boundaryFormLeft(trialElement: galerkin.element.Element1d, testElement: galerkin.element.Element1d):
        return elem1dUtils.evaluateBilinearFormAtBoundary(
            trialElement=trialElement, testElement=testElement, weight=lambda x: x * 0.0 + 1.0, B=0.0)

    def boundaryFormRight(trialElement: galerkin.element.Element1d, testElement: galerkin.element.Element1d):
        return elem1dUtils.evaluateBilinearFormAtBoundary(
    ...
    innerForm2 = lambda trialElement, testElement: elem1dUtils.integrateBilinearForm2(
        trialElement, testElement, weight=lambda x: 0*v(x), integrationPointsAmount=integrationPointsAmount)

    DGForm3 = lambda trialElement, testElement: elem1dUtils.evaluateDG_ErrorComponent(
        trialElement, testElement, weight=lambda x: -0*v(x))

    innerForm3 = lambda trialElement, testElement: elem1dUtils.integrateBilinearForm0(
        trialElement, testElement, lambda x: x * 0.0 - 1.0/l**2, integrationPointsAmount)
    ...
    return innerForm1, DGForm1, DGForm2, innerForm2, DGForm3, innerForm3,\
        boundaryFormLeft, boundaryFormRight, functional

# boundaryConditionsC_I = ['{"boundaryPoint": "0.0", "boundaryValue": 1e-3}',
#                       '{"boundaryPoint": "np.inf", "boundaryValue": 1}']
boundaryConditionsC_V = ['{"boundaryPoint": "0.0", "boundaryValue": 10.0}',
                      '{"boundaryPoint": "' + str(domainSize) + '", "boundaryValue": 1.0}']
...
def defectsCalculation():
    ...
    defectsC_V.setBilinearForm(innerForms=[C_V_Forms[0], C_V_Forms[3], C_V_Forms[5]],
                               discontinuityForms=[C_V_Forms[1], C_V_Forms[2], C_V_Forms[4]],
                                    boundaryForms=[C_V_Forms[-3], C_V_Forms[-2]])
    defectsC_V.setRHSFunctional(functionals=[C_V_Forms[-1]])
    ...
    # defectsC_V.
    plt.plot(defectsC_V.getMeshPoints(), sol)
    plt.show()
