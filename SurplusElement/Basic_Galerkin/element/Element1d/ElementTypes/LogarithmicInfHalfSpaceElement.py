...
class LogarithmicInfHalfSpaceElement:
    ...
    def __init__(self, interval, approxOrder, dirichletBoundaryConditions=None, parameters: approxParam = None):
        ...
        if self.interval[1] == np.inf:
            self.map = lambda x: 0.5 * s * np.log((3.0 + x)/(1.0 - x))
            self.inverseMap = lambda x: -1.0 + 2.0 * np.tanh(x / s)

            self.derivativeMap = lambda x: -0.5 * (x - 1.0) * (x + 3.0) / s
            self.inverseDerivativeMap = lambda x: - 2 * s / (-3.0 + 2.0 * x + x * x)
