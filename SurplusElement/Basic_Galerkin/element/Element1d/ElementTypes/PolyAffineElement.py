...
class PolyAffineElement:
    ...
    def evaluateExpansionDerivatives(self,
        ...
        if derivative == 1:
            ...
            return result
        if derivative == 2:
            chebD = spec.chebDiffMatrix(self.approxOrder, a=-1.0, b=1.0)
            basisMatrixL = spec.barycentricChebInterpolate(f=(chebD @ chebD @ self.refPointVal),
                                                          x=chebNodes, a=-1.0, b=1.0, axis=0) \
    ...
    def eval(self, x):
        ...
        x[x < self.interval[0]] = (self.interval[0] - 1.0) * np.ones(x[x < self.interval[0]].shape)
        x[x > self.interval[1]] = (self.interval[1] + 1.0) * np.ones(x[x > self.interval[1]].shape)
        # print(x)
        return spec.barycentricChebInterpolate(basisMatrix, x, a=self.interval[0], b=self.interval[1], axis=0)

    # def evalLimit(self, x, direction: int):
    #     """ Evaluates basis functions at points x
    #
    #                 Arguments:
    #                     x: evaluation points
    #                     direction: direction of limit. -1 - from the left, 1 - from the right
    #
    #                 Returns:
    #                     result: array with the shape: (*x.shape, degree of element)
    #     """
