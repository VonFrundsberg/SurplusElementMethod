...
from SurplusElement.Basic_Galerkin.element.Element1d.AuxClasses import ApproximationSpaceParameter as approxParam
import time as time
class RationalInfHalfSpaceElement:
    ...
    def evaluateExpansionDerivatives(self,
        ...
        if derivative == 1:
            ...
            x = self.inverseMap(np.atleast_1d(x))
            x[infValues] = 1.0
            evaluatedBasis = spec.barycentricChebInterpolate(basisMatrix, x, a=-1.0, b=1.0,
                                                             axis=0)
        ...
        if derivative == 2:
            ...
            # time.sleep(500)
            return (evaluatedBasisL + evaluatedBasisR) @ coefficients
        if derivative >= 3:
