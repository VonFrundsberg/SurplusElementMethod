from SurplusElement.Basic_Galerkin.element.Element1d.element1d import Element1d as belem
import numpy as np
...
def integrateBilinearForm2(trialElement: belem, testElement: belem,
    ...
    w, x = integr.reg_32_wn(a=-1.0, b=1.0, n=integrationPointsAmount)
    mappedX = trialElement.map(x)
    w = w * trialElement.inverseDerivativeMap(x)
    ...
    resultIntegrals = np.einsum('ijk,i->kj', D2, W)
    return resultIntegrals
...
def evaluateDG_centralFlux(trialElement: belem, testElement: belem,
    ...
    result = rightEvaluation + leftEvaluation
    return result
def evaluateDG_NaiveFlux(trialElement: belem, testElement: belem,
                                 weight, physicalBoundary: np.ndarray, eps: float = 1e-16):
    """Evaluates bilinear form of the type
    ...
    if trialElement.interval[1] in physicalBoundary:
        rightWeight *= 0
    IntervalL = trialElement.interval[0]
    IntervalR = trialElement.interval[1]

    # trialLL = trialElement.evalLim(IntervalL, -1)
    # trialLR = trialElement.evalLim(IntervalL, 1)
    # trialRL = trialElement.evalLim(IntervalR, -1)
    # trialRR = trialElement.evalLim(IntervalR, 1)
    #
    # testLL = testElement.evalLim(IntervalL, -1)
    # testLR = testElement.evalLim(IntervalL, 1)
    # testRL = testElement.evalLim(IntervalR, -1)
    # testRR = testElement.evalLim(IntervalR, 1)
...
def evaluateDG_upwind1(trialElement: belem, testElement: belem,
                                 weight, physicalBoundary: np.ndarray, eps: float = 1e-16):
    """Evaluates bilinear form of the type
            a(u, v) = weight(R) [u(R-)] * [v(R-) - v(R+)] +
             weight(L) [u(L-)] [v(L-) - v(L+)),
            where u(x)  are basis functions of trialElement
            and v(x)  are basis functions of testElement
            L and R are left and right (in inner limit) boundary of elementU interval.
    ...
    rightRef_LeftLim = np.nextafter(trialElement.interval[1], -np.inf)
    rightRef_RightLim = np.nextafter(trialElement.interval[1], np.inf)
    leftWeight = weight(trialElement.interval[0])
    rightWeight = weight(trialElement.interval[1])
    #
    if trialElement.interval[0] in physicalBoundary:
        leftWeight *= 0
    if trialElement.interval[1] in physicalBoundary:
    ...
    # leftTrialI = trialElement.eval(leftRef_RightLim)
    leftTestI = testElement.eval(leftRef_LeftLim) - testElement.eval(leftRef_RightLim)

    rightTrialI = trialElement.eval(rightRef_LeftLim)
    # rightTrialI = trialElement.eval(rightRef_RightLim)
    rightTestI = testElement.eval(rightRef_LeftLim) - testElement.eval(rightRef_RightLim)
    # print("elements: ", trialElement.interval, testElement.interval)
    # print(leftTrialI, leftTestI, rightTrialI, rightTestI)
    leftEvaluation = leftWeight * np.einsum("ij, ik -> jk",
                                            leftTrialI, leftTestI)
    rightEvaluation = rightWeight * np.einsum("ij, ik -> jk",
                                              rightTrialI, rightTestI)
    # print(leftEvaluation, rightEvaluation)
    nansLeft = np.isnan(leftEvaluation)
    leftEvaluation[nansLeft] = 0.0
    ...
    # if trialElement.interval[0] == testElement.interval[1]:
        ...
        # result = leftEvaluation - rightEvaluation
        # print(result)

    return result

def evaluateDG_upwind2(trialElement: belem, testElement: belem,
                                 weight, physicalBoundary: np.ndarray, eps: float = 1e-16):
    """Evaluates bilinear form of the type
            a(u, v) = weight(R) [u(R-)] * [v(R-) - v(R+)] +
             weight(L) [u(L-)] [v(L-) - v(L+)),
    ...
    if trialElement.interval[1] in physicalBoundary:
        rightWeight *= 0
    # if trialElement.interval[0] == 0.0 and testElement.interval[0] == 0.0:
    #     return np.einsum("ij, ik -> jk",
    #                                         trialElement.eval(rightRef_LeftLim), testElement.eval(rightRef_LeftLim))
    # if trialElement.interval[0] == 0.0 and testElement.interval[1] == 10.0:
    #     return np.einsum("ij, ik -> jk",
    #                      trialElement.eval(rightRef_LeftLim), testElement.eval(rightRef_RightLim))
    # return 0*np.einsum("ij, ik -> jk",
    #                      trialElement.eval(rightRef_LeftLim), testElement.eval(rightRef_RightLim))
    # leftTrialI = trialElement.eval(leftRef_LeftLim)
    leftTrialI = trialElement.eval(leftRef_RightLim)
    leftTestI = testElement.eval(leftRef_LeftLim) - testElement.eval(leftRef_RightLim)
...
def evaluateBilinearFormAtBoundary1(trialElement: belem, testElement: belem, weight, B: float):
    ...
    result = weightX * np.einsum("ij, ik -> jk", uX, vX)
    # print(result)
    return result
