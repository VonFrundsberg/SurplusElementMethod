...
from SurplusElement.Basic_Galerkin.element import Element1d, element
from SurplusElement.Basic_Galerkin.element.Element1d import element1d as element
from SurplusElement.Basic_Galerkin.element.Element1d.AuxClasses import DirichletBoundaryCondition
from SurplusElement.Basic_Galerkin.element.Element1d.AuxClasses import ApproximationSpaceParameter
import json
...
class GalerkinMethod1d:
    ...
    def initializeMesh(self, mesh):
        """Set up already made rectangular Mesh, which is an object of SurplusElementMethod/Basic_Galerkin/Mesh class
    ...
    def __calculateElements_LE(self):
        ...
        for i in range(elementsAmount):
            if verbose:
                print("element: ", self.elements[i].interval)
            ...
            if len(self.mesh.neighbours) > 0:
                for neighborNumber in self.mesh.neighbours[i]:
                    # if i < neighborNumber:
                        if verbose:
                            print("interaction: ", i, neighborNumber)
                        if discontiniousFormsAmount > 0:
                            self.matrixElements[neighborNumber][i] = self.discontinuityForms[0](self.elements[i], self.elements[neighborNumber]).T
                            # print(self.boundaryForms[0](self.elements[i], self.elements[neighborNumber]))
                            if verbose:
                                print("discontinuityForm interaction: \n", self.discontinuityForms[0](self.elements[i], self.elements[neighborNumber]))
                            for discontinuityFormNumber in range(1, discontiniousFormsAmount):
                                    self.matrixElements[neighborNumber][i] +=\
                                        (self.discontinuityForms[discontinuityFormNumber]
                                         (self.elements[i], self.elements[neighborNumber])).T
                        ...
                        # self.matrixElements[neighborNumber][i] = self.discontinuityForms[0](self.elements[neighborNumber],
                        #                                                                    self.elements[i])
                        # print("discontinuityForm interaction: \n",
                        #       self.discontinuityForms[0](self.elements[neighborNumber],
                        #                                  self.elements[i]))
                        # for discontinuityFormNumber in range(1, discontiniousFormsAmount):
                        #         self.matrixElements[neighborNumber][i] += \
                        #             self.discontinuityForms[discontinuityFormNumber](self.elements[neighborNumber], self.elements[i])
                    # else:
    ...
    def getRHS(self, elementIndex:int = 0):
        ...
        return np.hstack(self.functionalElements)
    def invertSLAE(self):
        """
    ...
    def solveSLAE(self):
        ...
        self.solution[ind] = solution
        # print(int(solution.shape[0]/2))
        # print(solution)
        # self.solution[int(self.solution.shape[0]/2):] += 1.0

        # solution = sp_linalg.lstsq(A.todense(), self.functionalElements)[0]
        # self.solution[ind] = solution
        # solution = sparse_linalg.gmres(A, self.functionalElements, rtol=1e-15, atol=)[0]
        # self.solution[ind] = solution
